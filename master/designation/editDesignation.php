	...
	<div id="main-content">
	 ...
	 if ( !empty($_POST)) {
			...
			// query
				...
				$sql = "SELECT * FROM DESIGNATION WHERE DESIGNATION_NAME = '$DESIGNATION_NAME' AND ID != '$ID'";
				$query =  $pdo->prepare( $sql );
				$query->execute();
                $rows = $query->fetch(PDO::FETCH_NUM);
               if($rows > 0) {
                    
					    echo "<div class='alert alert-info'>Designation<'$DESIGNATION_NAME' > already exists. No update done... </div>";
			...
			$sql = "UPDATE DESIGNATION 
					SET CREATED_DATE= NOW(), LAST_UPDATE_DATE= NOW(), DESIGNATION_NAME=?, DESCRIPTION=?
					WHERE ID=?";
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$q = $pdo->prepare($sql);
			$q->execute(array($DESIGNATION_NAME,$DESCRIPTION,$ID));
			...
			$ID = $_GET['id']; /** get the VISIT_DESIGNATION_NAMES ID **/
			        $pdo = Database::connect();
					$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);			
					$result = $pdo->prepare("SELECT * FROM DESIGNATION WHERE id= :userid");
		...
		?>
						<div class="widget">
							...
							<div class="widget-content">							   
                                ...
                                <div class="span3"><label>Designation<font color="#FF0000"> *</font></label><input type="text" autofocus  value="<?php echo $row['DESIGNATION_NAME'];?>" name="DESIGNATION_NAME" id="DESIGNATION_NAME" class="span3"></div>
