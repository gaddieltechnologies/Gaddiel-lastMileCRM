...
<?php
			...
			// query
			 $pdo = Database::connect();
				$sql = "SELECT * FROM CLIENT_CLASS WHERE CLIENT_CLASS = '$CLIENT_CLASS' AND ID != '$ID'";
				$query =  $pdo->prepare( $sql );
				$query->execute();
                $rows = $query->fetch(PDO::FETCH_NUM);
               if($rows > 0) {
                    	 echo "<div class='alert alert-info'>  Client Class<'$CLIENT_CLASS'> already exists. No update done. </div>";
						 

                 }
			  
		    else{
			$sql = "UPDATE CLIENT_CLASS 
					SET LAST_UPDATE_DATE= NOW(), CREATED_DATE= NOW(), CLIENT_CLASS=?, DESCRIPTION=?
					WHERE ID=?";
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$q = $pdo->prepare($sql);
			$q->execute(array($CLIENT_CLASS,$DESCRIPTION,$ID));
			echo "<div class='alert alert-success'> Successfully Updated. </div>"; 
			echo '<script type="text/javascript">
			window.location.href = "clientClass.php";
			</script>';
		}	
		}
			ob_start();
			$ID = $_GET['id']; /** get the CLIENT_CLASS ID **/
			        $pdo = Database::connect();
					$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);			
					$result = $pdo->prepare("SELECT * FROM CLIENT_CLASS WHERE id= :userid");
		...
		?>
						...
						<div class="widget">
							...
							<div class="widget-content">							   
                                ...
                                <div class="span3"><label>Client Class<font color="#FF0000"> *</font></label><input type="text" autofocus  value="<?php echo $row['CLIENT_CLASS'];?>" name="CLIENT_CLASS" id="CLIENT_CLASS" class="span3"></div>
