...
<!-- Navbar -->
	...
	<!-- /Navbar -->
	<form action="addDepartment.php" method="POST" onSubmit="return Validate();">
	<!-- Main content -->
	<div id="main-content">
		<!-- Container -->
		<div class="container">
					...
					<div class="span12">	
						<div class="widget">
							...
							<div class="widget-content">							   
                                ...
                                <div class="span4"><label>Department<font color="#FF0000"> *</font></label><input type="text" autofocus value="<?php echo !empty($DEPARTMENT_NAME)?$DEPARTMENT_NAME:'';?>" name="DEPARTMENT_NAME" id="DEPARTMENT_NAME" class="span4"></div>
...
<?php
		    ...
		    if ( !empty($_POST))
			{
               ...
               if($rows > 0) {
                    	echo "<script language=javascript>alert('Department already exits.')</script>";
                 }
			  
		    else{
				$pdo = Database::connect();
                $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $sql = "INSERT INTO DEPARTMENT (CREATED_DATE,LAST_UPDATE_DATE,DEPARTMENT_NAME,DESCRIPTION) values(now(), now(), ?, ?)";
                $q = $pdo->prepare($sql);
                $q->execute(array($DEPARTMENT_NAME ,$DESCRIPTION));
                Database::disconnect();
