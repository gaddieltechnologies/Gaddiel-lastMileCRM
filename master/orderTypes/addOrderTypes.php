					...
					<div class="span12">	
						<div class="widget">
							<div class="widget-header">
								<h3>Add Order Types</h3>
							</div>
							<div class="widget-content">							   
                                <div class="span3 hidefields"><label>Order Types Code</label><input type="text" name="ID" disabled="disabled"  class="span3"></div>
                                ...
                                <div class="span4"><label>Order Types<font color="#FF0000"> *</font></label><input type="text" autofocus value="<?php echo !empty($ORDER_TYPE)?$ORDER_TYPE:'';?>" name="ORDER_TYPE" id="ORDER_TYPE" class="span4"></div>
...
<?php
         ...
         $pdo = Database::connect();
				$sql = "SELECT * FROM ORDER_TYPES WHERE ORDER_TYPE = '$ORDER_TYPE'";
				$query =  $pdo->prepare( $sql );
				$query->execute();
                $rows = $query->fetch(PDO::FETCH_NUM);
               if($rows > 0) {
                    	echo "<script language=javascript>alert('Order Types already exits.')</script>";
                 }
                ...
                $sql = "INSERT INTO ORDER_TYPES (CREATED_DATE,LAST_UPDATE_DATE,ORDER_TYPE,DESCRIPTION) values(now(), now(), ?, ?)";
                $q = $pdo->prepare($sql);
                $q->execute(array($ORDER_TYPE ,$DESCRIPTION));
                Database::disconnect();
