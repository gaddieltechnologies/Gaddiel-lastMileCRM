...
$('#excelsubmit').click(function(event) {
        ...
        $.ajax({
                type: "POST",
                url: "orderDetailsReportsExcel.php",
...
<body onload="addDate();">
 ...
 <?php include($_SERVER['DOCUMENT_ROOT'].'/lastMileCRM/header.php'); ?>
	<!-- /Navbar -->
	<form action="orderDetailsReports.php" method="POST" onSubmit="return Validate();">
	<!-- Main content -->
	<div id="main-content">
		<!-- Container -->
		<div class="container">
				...
				<!-- Content -->
					<div class="span12">	
						...
						<div class="widget-content">							   
							   ...
							   <button type="submit" class="btn btn-success " name="pdfsubmit" value="pdf"><img src="../../resources/images/page_white_acrobat.png"/>&nbsp; Export PDF</button></div>  					
						</div>  	
						</div>
					</div>
				<!-- /Content -->
...
{	
	...
	$pdf->Cell(0,1,'ORDER BOOKED REPORT',0,1,'C');
	if ($empId=="ALL")
	{
	...
	if ($_POST["EMP_ID"] != "ALL")
				...
				{	
					...
					$clientid = $rowr['ClientId'];
							...
							{	
									...
									$pdf->Row(array($j,$rowvit['PRODUCT_CODE'], $rowvit['PROD_DESCRIPTION'],$rowvit['ORDER_QTY'], $rowvit['DELIVERY_DUE_DATE'],$rowvit['SPECIAL_INSTRUCTIONS']));
									$OrdCnt =  $OrdCnt +1;
        ...
        $filename="orderDetailsReports".$dated.".pdf";
		$pdf->Output($filename,'F');

			echo '<script type="text/javascript">
							window.location.href ="orderDetailsReports'.$dated.'.pdf";
							</script>';
