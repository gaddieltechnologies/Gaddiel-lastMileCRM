			...
			$header = <<<EOH
				...
				<!--[if gte mso 9]><xml>
				 <o:DocumentProperties>
				  <o:LastAuthor>Gaddiel Technologies</o:LastAuthor>
				...
				<!--[if gte mso 9]><xml>
				 <x:ExcelWorkbook>
				  <x:ExcelWorksheets>
				   <x:ExcelWorksheet>
					<x:Name>Employee Details</x:Name>
					<x:WorksheetOptions>
					 <x:Selected/>
		...
		*/
	    ...
	    function writeLines($color)
		{
			...
			foreach($color as $col)
				fwrite($this->fp,"<td class=xl24 width=64 style='color:#DC143C;font-size: 16px;' >$col</td>");
			fwrite($this->fp,"</tr>");
		}
		 function writeLinee($color)
		{
			if($this->state!="OPENED")
			{
				$this->error="Error : Please open the file.";
			...
			fwrite($this->fp,"<tr>");
			foreach($color as $col)
				fwrite($this->fp,"<td class=xl24 width=64 style='color:#DC143C;font-size: 14px;' >$col</td>");
			fwrite($this->fp,"</tr>");
		}
		 function writeLin($color)
		{
			if($this->state!="OPENED")
			{
				$this->error="Error : Please open the file.";
			...
			fwrite($this->fp,"<tr>");
			foreach($color as $col)
				fwrite($this->fp,"<td class=xl24 width=64 style='color:#8B008B;' >$col</td>");
			fwrite($this->fp,"</tr>");
		}
		 function write($color)
		{
			...
			fwrite($this->fp,"<tr>");
			foreach($color as $col)
				fwrite($this->fp,"<td class=xl24 width=64 style='color:#483D8B;font-size: 15px;' >$col</td>");
			fwrite($this->fp,"</tr>");
		}
		/*
