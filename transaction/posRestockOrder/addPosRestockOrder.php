...
function Validate(){
	...
	function addDate(){
		...
		document.getElementById('close_date').value = year + '-' + month + '-' + day +' '+ hour + ':'+ min;
		}
...
<!-- Navbar -->
	...
	<!-- /Navbar -->
	<form action="addPosRestockOrder.php" method="POST" enctype="multipart/form-data" onSubmit="return Validate();">
	<!-- Main content -->
	<div id="main-content">
		<!-- Container -->
		<div class="container">
							...
							<div class="widget-content">
								...
								<div class="span5"><label>Order Type<font color="#FF0000"> *</font></label>
								 <?php 
											...
											$pdo = Database::connect();
											$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
											$result = $pdo->prepare("SELECT ID,ORDER_TYPE FROM ORDER_TYPES");
											$result->execute();
											...
											for($i=0; $row = $result->fetch(); $i++){																	
												Database::disconnect();	
								?>												
											<option value="<?php  echo $row['ID'];?>"><?php  echo $row['ORDER_TYPE'];?> </option>						    
											<?php } echo '</select>';  ?>		
								</div>	
								...
								<div class="span5"><label>Product <font color="#FF0000"> *</font></label>
									 <?php 
											...
											$pdo = Database::connect();
											$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
											$result = $pdo->prepare("SELECT ID,PRODUCT_NAME FROM PRODUCT_MASTER");
											$result->execute();
								...
								<div class="span6"><label>UOM<font color="#FF0000"> *</font></label><input type="text" value="<?php echo !empty($EMP_NAME)?$EMP_NAME:'';?>" name="ORDER_UOM"  id="order_uom" class="span6"></div>															
			            		<div class="span5"><label>Order Qty<font color="#FF0000"> *</font></label><input type="text" value="<?php echo !empty($EMP_NAME)?$EMP_NAME:'';?>" name="ORDER_QTY"  id="order_qty" class="span5"></div>				
								...
								<div class="span3"><label>Close Date Time<label><input type="text" maxlength="20" value="" name="CLOSE_DATE" id="close_date" class="span3 pickerfield"></div>
								<div class="span8"><label>Comments</label><textarea value="" name="RESTOCK_COMMENTS" id="RESTOCK_COMMENTS"  class="span8"></textarea></div>	
									...
									<div class="container">                  
											<center>
												<div class="btn-group btn-group-justified">                      
													...
													<a href="posRestockOrder.php" class="btn btn-default">
...
<?php
		  ...
		  if ( !empty($_POST))
			{
				...
				$RESTOCK_COMMENTS = $_POST['RESTOCK_COMMENTS'];						
				...
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				 $result = $pdo->prepare("SELECT CLIENT_NAME,CLIENT_EMAIL_ID,CLIENT_LOC FROM CLIENT_DETAILS WHERE ID = '$CLIENT_NAME'");
				$result->execute();									
				Database::disconnect();
				$rowclt = $result->fetch();
		...
		$date = date("Y/m/d");
        $time =  date("h:i:sa");		
		$Message ="<p>Please restock the following products in the following retail location</p><br>
		<p>PoS Restock Location: Client - $clientName, $clientLocation</p>
		<table border='1' width='100%' >
		  ...
		  <tr>
			...
			<td>$ORDER_QTY</td>
			<td>$ORDER_UOM</td>
			<td>$RESTOCK_COMMENTS</td>			
		  </tr>
		</table>
		<p>Regards,</p>";
					 
         $FromEmail =   'arunraj.balu@gaddieltech.com'; //sumithnets@yahoo.com
		 $Subject   =   "CLIENT NAME - $clientName, EMPLOYEE NAME - $empname , DATE - $date";  		
        ...
        // $mail->AddAttachment( $file_to_attach , $filename, 'base64','mime/type');
		 ...
		 if($mail->Send()){
          echo "<div class='alert alert-success'> Email sent to : $empID,$email1,$email2,$email3,$email4,$email5,$email6 </div>"; 
			    ...
			    $pdo = Database::connect();
                $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);			
                $sqlq = "INSERT INTO POS_RESTOCK_ORDER (CREATED_DATE,BOOKING_EMPLOYEE_ID,ORDER_TYPE_ID,CLIENT_ID,PRODUCT_ID,ORDER_QTY,ORDER_UOM,BOOKING_DATE,CLOSE_DATE,RESTOCK_DUE_DATE,RESTOCK_COMMENTS) values(now(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $q = $pdo->prepare($sqlq);
                $q->execute(array($EMP_NAME,$ORDER_TYPE,$CLIENT_NAME,$PRODUCT_NAME,$ORDER_QTY,$ORDER_UOM,$BOOKING_DATE,$CLOSE_DATE,$DELIVERY_DUE_DATE,$RESTOCK_COMMENTS));
                Database::disconnect();	
